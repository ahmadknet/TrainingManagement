...
import { Course, CourseForm, ResponseDto } from './course.models';
...
export class CoursesComponent implements OnInit {
  ...
  protected loadCourses(): void {
    ...
    this.courseApi
      ...
      .subscribe({
        ...
        error: (err) => {
          this.error = this.extractHttpError(err, 'Courses could not be loaded.');
        }
      });
  }
  ...
  protected saveCourse(): void {
    ...
    request.pipe(finalize(() => (this.saving = false))).subscribe({
      ...
      error: (err) => {
        this.error = this.extractHttpError(err, 'Course could not be saved.');
      }
    });
  }
  ...
  protected deleteCourse(course: Course): void {
    ...
    this.courseApi
      ...
      .subscribe({
        ...
        error: (err) => {
          this.error = this.extractHttpError(err, 'Course could not be deleted.');
        }
      });
  }
